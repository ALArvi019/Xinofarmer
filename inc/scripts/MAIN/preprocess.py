...
class PreProcessImage:
    ...
    def pp_image(self, mapa_completo):
        border_size = 60
        mapa_completo = cv.copyMakeBorder(mapa_completo, border_size, border_size,
                                          border_size, border_size, cv.BORDER_CONSTANT, None, value=[0, 0, 0])
        gray_mapa_completo = cv.cvtColor(mapa_completo, cv.COLOR_BGR2GRAY)
        _, binary_image = cv.threshold(
            gray_mapa_completo, 254, 255, cv.THRESH_BINARY)
        mapa_completo = cv.bitwise_and(mapa_completo, cv.cvtColor(
            cv.bitwise_not(binary_image), cv.COLOR_GRAY2BGR))

        return mapa_completo

    def is_touching_edge(self, image, x, y, radius):
